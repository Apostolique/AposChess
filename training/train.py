...
def parse_args():
    ...
    p.add_argument("--rescale", action="store_true",
                   help="honour --scale even on a warm start, compensating the head so the "
                        ...
                        "archived champions carry scale=600 and 0 of the 32 experiment "
                        "tracks ever set one, across 497 cycles. The squash also "
                        ...
                        "Measured by `npm run data:audit` on the 34.8M-position set "
                        "(2026-08-08) — run it for the current figures rather than trusting "
                        "these: |v| piles up against the ceiling (6.45%% in 400-450, 11.09%% "
                        "in 450-500, 13.05%% in 500-550, 10.15%% in 550-600, then 0.05%% in "
                        "600-650), so 41.3%% of labels sit in the saturation zone above "
                        "0.67*scale and 48.0%% are saturated or off the scale entirely once "
                        "the 6.8%% mate scores are counted. Mean local tanh gain "
                        "1 - (v/scale)^2 is 0.616, and 0.18%% of labels read exactly +/-600 "
                        "— the eval has run out of scale and has nothing more to say. Those "
                        "positions contribute loss but little gradient, which is also the "
                        "simplest explanation for val loss having stopped tracking strength.")
